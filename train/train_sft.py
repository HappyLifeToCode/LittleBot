...
from model.config import MiniBotConfig
from model.model_minibot import MiniBotForCausalLM
from dataset.sft_dataset import \
    SFTDataset  # [SFT] pretrain 用 PretrainDataset(.bin)，SFT 用 SFTDataset(jsonl + 只算 assistant loss)
...
def train_epoch(epoch, loader, iters, start_step=0, swanlab=None, total_steps=None, warmup_steps=None,
    ...
    for step, (input_ids, labels) in enumerate(loader, start=start_step + 1):
        ...
        if step % args.log_interval == 0 or step == iters - 1:
            ...
            if swanlab: swanlab.log({"loss": current_loss, "learning_rate": current_lr, "eta_time": eta_min},
                                    step=global_step)
        ...
        if args.enable_eval and getattr(args, "eval_interval",
                                        0) > 0 and global_step % args.eval_interval == 0 and is_main_process():
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="MiniBot SFT Training")
    ...
    parser.add_argument("--learning_rate", type=float, default=2e-5,
                        help="初始学习率（SFT 推荐 1e-5 ~ 1e-4，从预训练继续可用 5e-5）")
    ...
    parser.add_argument("--tokenizer_path", type=str, default="../tokenizer_15k",
                        help="tokenizer 路径")  # [SFT] 新增：SFTDataset 需要 tokenizer
    ...
    parser.add_argument("--swanlab_project", type=str, default="MiniBot-SFT", help="swanlab 项目名")
    ...
    parser.add_argument("--eval_interval", type=int, default=1000,
                        help="每隔多少 step 跑 mini_bench（0=关闭），用当前模型推理+DeepSeek Judge 打分")
    parser.add_argument("--judge_api_key", type=str, default='',
                        help="Judge API Key（可直接传入或从环境变量 DEEPSEEK_API_KEY 读取）")
    ...
    # ========== 2. 配置目录、模型参数、检查 ckp ==========
    lm_config = MiniBotConfig(hidden_size=args.hidden_size, num_hidden_layers=args.num_hidden_layers)
    ...
    if args.from_weight != 'none' and os.path.exists(args.from_weight):
        ...
        model = MiniBotForCausalLM(lm_config)
    ...
    if args.enable_eval and getattr(args, "eval_interval",
                                    0) > 0 and is_main_process() and start_epoch == 0 and start_step == 0:
