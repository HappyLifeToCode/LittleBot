...
from .config import MiniBotConfig
...
class Attention(nn.Module):
    ...
    def __init__(self, args: MiniBotConfig):
        super().__init__()
...
class MiniBot(nn.Module):
    """
    MiniBot 模型主体（Decoder-only Transformer）
    """
...
class MiniBotForCausalLM(PreTrainedModel, GenerationMixin):
    """
    MiniBot 因果语言模型（用于文本生成）
    在 MiniBotModel 基础上添加 Language Modeling Head
    """
    config_class = MiniBotConfig

    def __init__(self, config: MiniBotConfig = None):
        self.config = config or MiniBotConfig()
